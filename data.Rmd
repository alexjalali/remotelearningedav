...
lower_bound <- min_percent_remote + (max_percent_remote - min_percent_remote) / 3
upper_bound <- max_percent_remote - (max_percent_remote - min_percent_remote) / 3

# Create the new categorical variable using the cut() function
df_super$remote_cat <- cut(df$PercentRemote, breaks = c(min_percent_remote, lower_bound, upper_bound, max_percent_remote), labels = c("Low", "Middle", "High"))
```
...
```{r temp missing value analysis}
#THIS IS NOT HOW I PLAN TO DO THIS I JUST NEED TO DO THIS TEMPORARILY SO THE GRAPH I AM TRYNA MAKE WORKS LOL
df[is.na(df)] <- 0
```
