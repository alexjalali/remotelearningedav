...
```{r making PercentRemote categorical}
...
### Our Final Dataset, first 10 Observations
```{r}
head(df_super, 10)
```

### Our Final Dataset, last 10 Observations
```{r}
tail(df_super, 10)
```

### Saving CSV to csv for interactive component
```{r saving to csv "dataClean"}
write.csv(df_super, file="./datasets/DataClean.csv")
```

### Saving Consolidated and Cleaned Dataset into Datasets df and df_total
...
```{r fig.height=4}
A<-subset(df_super_MVA,Category=="Asian")
Aplot <- gg_miss_case(A,facet=District) + labs(title="Ethnicity: Asian") + theme(plot.title = element_text(size = 8))

W<-subset(df_super_MVA,Category=="White")
Wplot <- gg_miss_case(W,facet=District)+ labs(title="Ethnicity: White")+ theme(plot.title = element_text(size = 8))
